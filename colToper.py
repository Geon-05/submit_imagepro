...
# 모델 로드
model_path = "/home/zqrc05/project/imagepro/test/model/grayTocol/12_05_best_model_7_0.043.pth"
...
output_dir = "/home/zqrc05/project/imagepro/test/output_colToper_12"      # 결과 저장 폴더
...
for filename in os.listdir(test_images_dir):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        # 모델 추론
        with torch.no_grad():
            output = generator(inp_tensor, mask_tensor)  # (1,3,H,W)

        # 결과 텐서 -> 이미지
        output_img = output.squeeze(0).cpu()  # (3,H,W)
        output_img_pil = T.ToPILImage()(output_img)

        # 결과 저장 (원본 파일명에 맞춰 저장)
        save_path = os.path.join(output_dir, filename)
        output_img_pil.save(save_path)
        print(f"{filename} 처리 완료 -> {save_path}에 저장")
